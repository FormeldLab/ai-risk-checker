...
PROHIBITION — these statements are BANNED if the pre-extracted value is not "명시 없음":
...
■ market_entry (1–10, 높을수록 진입 어려움)
  ...
  보정 (+1): differentiation_source가 "명시 없음" OR 기존 제품과 동일 기능만 나열
...
■ differentiation (1–10, 높을수록 차별화 실현 용이)
  ...
  보정 (-1): acquisition_channel이 "명시 없음"
  범위: 3–8
...
Field definitions:
- target_customer:        Who pays? (e.g. "음식점 사장, 자영업자")
- revenue_model:          How is money made? Include price if stated. (e.g. "월 39,000원 구독")
- acquisition_channel:    How are customers reached/acquired? Look for: 광고, 바이럴, SNS, 커뮤니티, 영업, 플랫폼 키워드. (e.g. "네이버 스마트플레이스 광고, 소상공인 커뮤니티 바이럴")
- operating_model:        How is the service delivered? (e.g. "SaaS, POS 연동")
- differentiation_source: What makes this different? (e.g. "POS 연동으로 실제 판매 데이터 자동 수집")
- repeat_purchase_basis:  Why do customers pay again? (e.g. "월 구독 자동 갱신")
- expansion_path:         How does this scale? (e.g. "명시 없음")

Output this exact JSON shape:
{
  "target_customer":        "직접 인용",
  "revenue_model":          "직접 인용",
  "acquisition_channel":    "직접 인용",
  "operating_model":        "직접 인용",
  "differentiation_source": "직접 인용",
  "repeat_purchase_basis":  "직접 인용",
  "expansion_path":         "직접 인용"
}

RULES:
- Quote directly from the description. Use exact phrases when available.
- Only write "명시 없음" when the field is truly absent from the description.
- Do NOT infer from the business name alone — read the full description.
- For acquisition_channel: look for ANY mention of marketing, advertising, communities, platforms, word-of-mouth, sales methods.
- If description says 자영업자/소상공인/사업자 are customers → target_customer must reflect this B2B context.
`;
...
function computeScores(extraction) {
  ...
  if (differentiation_source === "명시 없음") me += 1;
  ...
  let diff;
  if (differentiation_source === "명시 없음") {
    diff = 3;
  ...
  if (acquisition_channel === "명시 없음") diff -= 1;
...
app.post("/analyze", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      extraction = {
        target_customer: "명시 없음", revenue_model: "명시 없음",
        acquisition_channel: "명시 없음", operating_model: "명시 없음",
        differentiation_source: "명시 없음", repeat_purchase_basis: "명시 없음",
        expansion_path: "명시 없음"
      };
    }
...
위 추출값이 "명시 없음"이 아닌 경우, 해당 항목에 대해 "채널 전략 부재", "수익 모델 불명확", "타겟 미정" 등의 표현을 사용하지 말 것.
`;
